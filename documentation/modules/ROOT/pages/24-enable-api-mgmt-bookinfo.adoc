...
----
curl https://3scale-admin.apps.cluster-pmjd8.pmjd8.sandbox783.opentlc.com/admin/api/services/4/proxy/configs/production/latest.json?access_token=OjcCkLtcZKopD1K9 | jq '.proxy_config.content.backend_authentication_value'
----

Copy and note down the Service Access Token obtained
===============================

. Create a Custom Resource Definition file for the Service Mesh Extension with name `ServiceMeshExtension_bookinfo-oidc.yaml` using vim or any other editor on the CLI. Copy paste the below yaml into the file and replace the access token, service token, product id values and shown in the image below and save it. 
+
[.lines_7]
[.console-input]
[source, yaml,subs="+macros,+attributes"]
...
. Make sure you choose and fill in the correct values as shown below.
 ...
 - Access Token URL: https://keycloak-keycloak.%CLUSTER_WILDCARD_URL%/auth/realms/threescale/protocol/openid-connect/auth
